...
public class Server {
 ...
 //   roomCur = world.spawnRoom;
  ...
  public void onMessage(String mes, Session ses) {
    ...
    case "kick": break;
    case "upg":
    case "upgrade": upgrade(parts); break;
    case "flee": self.flee(); break;
    
    default:
      if(self.getActions().size()>0){
        self.broadcast("you are still in combat!\ntry flee");
        break;
      }
  ...
  public void look(String[] parts){
    if (parts.length == 2) {
      ...
      if (search != null)
        ...
        String ex = self.getContainer().getExits().findExit(parts[1]);
        if (!ex.isEmpty()) {
...
//          System.out.println(old[0]==null?"null":old[0].getShortName());
  ...
  */
  public void upgrade(String[] parts){
    if(parts.length==1) self.broadcast("upgrade which stat?!",parts[0]);
      ...
      if(xp>=cap){
          ...
          self.getStats().setMaxHealth(self.getStats().getMaxHealth()+1);
