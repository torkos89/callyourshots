 ...
 //   roomCur = world.spawnRoom;
  ...
  public void onMessage(String mes, Session ses) {
    ...
    if (mes.startsWith("|:|")) {
      ...
      try {
        broadcast(player + " logged in");  //TODO: make global
        Thread.sleep(50);
        broadcast(self.getContainer().toString());
    ...
    case "inventory": broadcast(self.getInventory().toString(),mes); break;   
    case "eq":
    case "equipment": broadcast(self.getEquipment().toString(),mes); break;    
    case "l":
    case "lo":
    case "loo":
    ...
    default:
      ...
      if (!exit.isEmpty()){  //self.getContainer().getExits().hasExit(parts[0])) {
        ...
        broadcast(player+" enters the room", next.getInventory()); 
        broadcast(player+" leaves "+exit, oldRoom.getInventory());
        self.setContainer(next);
        broadcast(self.getContainer().toString(),parts[0]);
      } else
        broadcast("invalid command: " + mes,mes);
    }
  }
  @OnClose
  public void onClose(Session ses){
   // sessions.remove(ses.getId());
    ...
    broadcast(ses.getUserProperties().get("name").toString() + " logged out"); //TODO: make global
  }
  ...
  public void look(String[] parts){
    if (parts.length == 2) {
      ...
      if (search != null)
        broadcast(search.toString(),parts[0]);
      else{
        if (self.getContainer().getExits().hasExit(parts[1])) {
          broadcast(self.getContainer().getExits().getExit(parts[1]).toString(),parts[0]);
        }
        else broadcast("cant find " + parts[1],parts[0]);
      }
    } else
      broadcast(self.getContainer().toString(),parts[0]);
  }
  ...
  public void get(String[] parts){
    if (parts.length == 1)
      broadcast("get what?!",parts[0]);
    else if (parts.length == 2) {
      Entity item = self.getContainer().getInventory().remove(parts[1]);
      if (item != null){
        item.setContainer(self);
        broadcast("you pick up " + item.getShortName(),parts[0]+" "+parts[1]);
        broadcast(player+" picks you up",item);
        broadcast(player+" picks up "+item.getShortName(),self.getContainer().getInventory(),item);
      }
      else
        broadcast("you can't find " + parts[1],parts[0]+" "+parts[1]);
    } else {
      Entity container = self.getInventory().get(parts[2]);
      if (container == null)
        container = self.getContainer().getInventory().get(parts[2]);
      if (container != null) {
        if(container.hasStats()) broadcast(container.getShortName()+" doesn't let you take anything",parts[0]+" "+parts[1]+" "+parts[2]);//TODO: increase anger, reduce repore / start combat
        else{
          Entity item = container.getInventory().remove(parts[1]);
          if (item != null){
            item.setContainer(self);
            broadcast("you get " + item.getShortName() + " from " + container.getShortName(),parts[0]+" "+parts[1]+" "+parts[2]);
            broadcast(player+" picks you up",item);
            broadcast(player+" picks up "+item.getShortName(),self.getContainer().getInventory(),item);
          }
          else broadcast("you can't find " + parts[1] + " in " + container.getShortName(),parts[0]+" "+parts[1]+" "+parts[2]);
        }
      }else broadcast("you can't find " + parts[2],parts[0]+" "+parts[1]+" "+parts[2]);
    }
  }
  
  public void drop(String[] parts){
    if(parts.length==1) broadcast("drop what?!",parts[0]);
    Entity item = self.getInventory().get(parts[1]);
    if(item==null) broadcast("you dont have item "+parts[1],parts[0]+" "+parts[1]);
    else{
      self.getInventory().remove(parts[1]).setContainer(self.getContainer());
      broadcast("you drop "+item.getShortName()+" in the room",parts[0]+" "+parts[1]);
      broadcast(player+" drops "+item.getShortName()+" in the room", self.getContainer().getInventory(),item);
      broadcast(player+" drops you",item);
    }
  }

  public void put(String[] parts){
    if(parts.length==1) broadcast("put what, where?!",parts[0]);
    else if(parts.length==2)broadcast("put "+parts[1]+", where?!",parts[0]+" "+parts[1]);
    else{
      Entity search = self.getInventory().get(parts[1]);
      if(search==null) broadcast("can't find "+parts[1],parts[0]+" "+parts[1]);
      else{
        Entity container = self.getInventory().get(parts[2]);
        if(container==null) container = self.getContainer().getInventory().get(parts[2]);
        if(container==null) broadcast("can't find "+parts[2],parts[0]+" "+parts[1]+" "+parts[2]);
        else{
         self.getInventory().remove(parts[1]).setContainer(container);
         broadcast("you put "+search.getShortName()+" in "+container.getShortName(),parts[0]+" "+parts[1]+" "+parts[2]);
        }
      }
    }
  }
  ...
  public void wear(String[] parts){
    if(parts.length==1) broadcast("wear what?!",parts[0]);
    else if(parts.length==2){
...
//          System.out.println(old[0]==null?"null":old[0].getShortName());
          if(old==null||old[0]==null) broadcast("you wear "+item.getShortName(),parts[0]+" "+parts[1]);
          else{
            for(Entity e : old) self.getInventory().add(e);
            if(old.length==1)broadcast("you remove "+old[0].getShortName()+" and wear "+item.getShortName(),parts[0]+" "+parts[1]);
            else if(old.length==2){
              if(old[1]==null) broadcast("you remove "+old[0].getShortName()+" and wear "+item.getShortName(),parts[0]+" "+parts[1]);
              else if(old[0]==null) broadcast("you remove "+old[1].getShortName()+" and wear "+item.getShortName(),parts[0]+" "+parts[1]);
              else broadcast("you remove "+old[0].getShortName()+" and "+old[1].getShortName()+" and wear "+item.getShortName(),parts[0]+" "+parts[1]);
            }
          }
        }
        else broadcast("you can't wear "+item.getShortName(),parts[0]+" "+parts[1]);
      }
      else broadcast("you can't find "+parts[1],parts[0]+" "+parts[1]);
    }
  }
  
  public void remove(String[] parts){
    if(parts.length==1) broadcast("remove what?!",parts[0]);
    else if(parts.length==2){
      Entity item = self.getEquipment().remove(parts[1]);
      if(item!=null){
        self.getInventory().add(item);
        broadcast("you remove "+item.getShortName(),parts[0]+" "+parts[1]);
      }
      else broadcast("you're not wearing "+parts[1],parts[0]+" "+parts[1]);
    }
  }
  
  public void attack(String[] parts){
    if(parts.length==1) broadcast("attack what?!",parts[0]);
    else if(parts.length==2){
      Entity target = self.getContainer().getInventory().get(parts[1]);
      if(target==null) broadcast("you can't find "+parts[1],parts[0]+" "+parts[1]);
      else if(!target.hasStats()) broadcast("you can't attack "+target.getShortName(),parts[0]+" "+parts[1]);
      else{
  ...
  public void kick(String[] parts){
    ...
    else broadcast("you dont have a target");
  ...
  public void improve(String[] parts){
    if(parts.length==1) broadcast("improve what stat?!",parts[0]);
  ...
  public void say(String mes){
    broadcast(self.getShortName()+" says '"+mes.substring(4)+"'",self.getContainer().getInventory());
    broadcast("you say '"+ mes.substring(4)+"'");
  }
  public void tell(String[] parts, String mes){
    if(parts.length==1) broadcast("tell to who?!",parts[0]);
    else if(parts.length>=2){
      Entity target = null;
      ...
      if(target==null){
        ...
        broadcast("you can't find "+parts[1]+" to tell to");
      }
      
      else{

        broadcast(self.getShortName()+" tells you '"+mes.substring(parts[0].length()+parts[1].length()+2)+"'",target);
        broadcast("you tell '"+mes.substring(parts[0].length()+parts[1].length()+2)+"' to "+parts[1]);
      }
    }
  }
  /*
